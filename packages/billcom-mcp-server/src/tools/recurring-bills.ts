...
export function registerRecurringBillTools(server: McpServer): void {
  server.tool(
    ...
    "List recurring bills from your Bill.com account. Supports filtering and sorting.",
    {
      ...
      filters: z.string().optional().describe("Field name, operator, and value for filtering. Format: field:op:value or compound field1:op:value,field2:op:value. Example: vendorId:eq:abc123"),
    },
        ...
        content: [
          {
            type: "text" as const,
            text: JSON.stringify(
              { recurringBills: data.results, nextPage: data.nextPage, prevPage: data.prevPage },
              null,
              2
            ),
          },
