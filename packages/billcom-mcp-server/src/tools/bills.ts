...
export function registerBillTools(server: McpServer): void {
  server.tool(
    "list_bills",
    "List bills from your Bill.com account. Shows outstanding and paid bills with amounts, due dates, and vendors. Supports filtering and sorting.",
    {
        ...
        content: [
          {
            type: "text" as const,
            text: JSON.stringify(
              { bills: data.results, nextPage: data.nextPage, prevPage: data.prevPage },
              null,
              2
            ),
          },
